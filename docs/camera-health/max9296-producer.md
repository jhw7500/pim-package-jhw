...
`BLOCKED/REMOTE_PATH_UNAVAILABLE` 만 기록했다. 공유 domain 도 정확히 해제된다(예: ch2 제거 시
`phys=11`, `active=3`).

> **이 표의 "반대 MAX9296 0초" 는 케이블이 빠져 있는 동안만 본 값이다.** 그 구간에 한해
> 12회 전체에서 반대 deserializer 의 `FAIL` 오판은 **0초**였다. 다만 아래 "cable 추가 회차"
> 에서 **모듈 재초기화 구간**까지 보니 반대 칩에 짧은 `FAIL` 이 나왔다. 12회 당시 분석이
> 그 구간을 보지 않았을 뿐이며, 동작이 바뀐 것은 아닐 가능성이 높다.
...
### cable 추가 회차 7회 (2026-08-18, pim-camera-v016)

ch1 ×3(2회 계획 + 1회 비의도), ch2 ×2, ch3 ×2 를 추가했다. 이번에는 gstApp producer 를
포함한 세 producer 를 모두 켠 구성이고, 커널 로그를 **관측 시작 시점부터 파일로 받았다**.

| # | 뽑은 채널 | 반대 칩 `FAIL` | 그때 같은 칩 peer | 커널 로그 커버 |
|---|---|---|---|---|
| 1 | ch1 | 0초 | — | ✗ (회전) |
| 2 | ch1 | **ch2 10초** | ch3 `OK` 유지 | ✗ (회전) |
| 3 | ch2 | **ch0 14초** | ch1 `OK` 유지 | ✗ (2초 차) |
| 4 | ch2 | 0초 | — | ✓ |
| 5 | ch1 (비의도) | 0초 | — | ✓ |
| 6 | ch3 | 0초 | — | ✓ |
| 7 | ch3 | 0초 | — | ✓ |

`✗ (회전)` 은 journald 보존 한계(약 16분)로 해당 구간 커널 로그가 이미 소실됐다는 뜻이고,
`✗ (2초 차)` 는 커널 로그 수집기를 그 사건이 끝난 2초 뒤에 띄워 놓쳤다는 뜻이다.

**반대 칩에 짧은 `FAIL` 이 7회 중 2회 나왔다. 원인은 미확정이다.**

정황은 "재초기화 중 실제 링크 상태"를 가리킨다:

- 두 사례 모두 모듈 리로드(`DES_I2C_FAIL` → 복귀) **직후**에만 나타났다.
- 그 구간 내내 **같은 칩의 짝 채널은 `OK`** 였다. 칩 단위로 뭉개는 오판이면 둘 다 `FAIL`
  이어야 한다.
- 그 구간 deserializer 는 `OK/NONE` 이었다. 칩과 통신이 되는 상태에서 읽은 per-link 값이다.
- `physical_present_mask` 가 `9`(ch0·ch3) → `12`(ch2·ch3) 처럼 순차 변화했다. 링크가 하나씩
  올라오는 모습이다.

그러나 **하필 두 사례 모두 커널 로그 밖이라 독립 확인을 못 했다.** 사례 3 은 커널 수집기가
2초 늦게 시작해 놓쳤다. 재현되면 커널의 `Unregistered all entities` → `shared Init` →
`Registered ... 1-0048/2-0048` → `set_power` → `apply_cached_controls` 순서와 초 단위로
대조해 확정해야 한다.

운영상 함의: 재링크 중과 진짜 링크 다운이 같은 `FAIL/GMSL_LINK_DOWN` 으로 보인다. recovery
권한을 붙일 때 이 10~14초를 어떻게 다룰지 정해야 한다.

**샘플러 타임라인은 커널 기록으로 5회 검증됐다** (오차 1~2초):

| 커널 `Unregistered all entities` | 이 시험의 리로드 검출 |
|---|---|
| 08:20:13 / 08:32:12 / 08:35:16 / 08:51:15 / 08:54:15 | 08:20:15 / 08:32:13 / 08:35:17 / 08:51:17 / 08:54:17 |

**누적 회차**: ch0 5 / ch1 6 / ch2 5 / ch3 3 (기준 각 10회).

**계측 함정 (다음 사람을 위해)**

- journald 는 이 보드에서 약 16분만 보존한다. **사후에 뜨면 이미 사라진다.** 정작 확인해야
  할 두 구간을 그렇게 잃었다. 보드 계측은 커널 로그를 처음부터 파일로 받아야 한다.
- `pgrep -f` / `pkill -f` 는 자기 명령줄에 매칭된다. 수집기가 안 떴는데 떴다고 오판했고,
  나중엔 `pkill` 이 자기 ssh 세션을 먼저 죽였다. `ps -eo pid,args` 로 확인하고 PID 로 지정한다.
...
1. cable 시험 회차를 doc 원문 기준(각 10회)으로 채운다. **현재 ch0 5 / ch1 6 / ch2 5 / ch3 3**
   (2026-08-18 에 ch1 ×3, ch2 ×2, ch3 ×2 추가 — 위 "cable 추가 회차" 참조).
   더불어 그 회차에서 나온 **반대 칩 짧은 `FAIL` 2건의 원인을 확정**해야 한다.
...
4. ~~gstApp `PRODUCER_MALFORMED` 단발의 사유를 확정한다.~~ **확정·수정 완료** — 사유는
   `future_monotonic_time` 이었다. aggregator 가 루프 앞에서 `now_ms` 를 찍고 그 뒤에 파일을
   읽어, 그 사이에 publish 된 스냅샷이 `age_ms < 0` 으로 거부됐다. 읽기 구간이 설명하는
   250ms 까지는 신선한 것으로 받아들이도록 고쳤다(`FUTURE_SKEW_TOLERANCE_MS`).
