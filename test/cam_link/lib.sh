...
# 종료 조건은 컬럼 0 의 '}' 단독 행으로 좁힌다. '}' 로 시작하는 다른 행(그룹 명령
# 닫기 등)에서 끊기면 '비어있지 않은 절반짜리' 파일이 나와 -s 검사를 통과하므로,
# 추출 결과가 실제로 함수로 파싱되는지까지 확인한다.
t_extract_func() {
    local src="$1" fn="$2" out="$3"
    awk -v f="^${fn}[[:space:]]*\\\\([[:space:]]*\\\\)[[:space:]]*\\\\{" \
        '$0 ~ f {p=1} p {print} p && /^\}$/ {exit}' "$src" > "$out"
    [ -s "$out" ] || { echo "함수 추출 실패: $fn ($src)" >&2; return 1; }
    ( set +e; source "$out" >/dev/null 2>&1; [ "$(type -t "$fn")" = "function" ] ) || {
        echo "함수 추출 불완전: $fn — 닫는 '}' 를 못 찾았을 수 있다 ($src)" >&2
        return 1
    }
}
...
# $3=호출 횟수를 기록할 파일(선택)
# 주의: 이 스텁은 읽기(w2@...r1)와 쓰기(w3@...)를 구분하지 않는다. 즉 호출 횟수에
# 리셋 쓰기도 포함되며, 반환값도 양쪽 같다. 호출 수 단정을 계산할 때 유의할 것.
t_make_stubs() {
