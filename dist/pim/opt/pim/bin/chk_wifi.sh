...
if [ $i == 3 ] ; then
	...
	if [ "$WLAN0_FIND" != "wlp1s0" ]; then
		...
		if [ "$val" == "y" ]; then
			...
			if [ -z "$WIFI_SSID" ] ; then
				logger -p local0.priority -t $tag [CHK] WIFI CONNECT ERR
				echo "${timestamp} WIFI ERR" >> ${FLAG_PATH}/err_wifi.log
			else
				...
				logger -p local0.priority -t $tag [CHK] WIFI SSID : $WIFI_SSID
				logger -p local0.priority -t $tag [CHK] WIFI IP ADDR : $IP_ADDR
				logger -p local0.priority -t $tag [CHK] WIFI Signal : $WIFI_SIG dBm
				WIFI_PING=$(ping $IP_ADDR -c 3 -W 3 -s 1000)	
				if [[ $WIFI_PING != *"$_success_value"* ]]; then
					logger -p local0.priority -t $tag [CHK] WIFI PING ERR
					echo "${timestamp} WIFI ERR" >> ${FLAG_PATH}/err_wifi.log				
				fi
			fi
		fi		
