...
function LogWrite() {
	...
	echo "${timestamp}|$1" >> "${_root_path}/wificheck.log"
}

function LogWrite_WiFiRegion() {
	local otp_dmesg=`dmesg | grep "Firmware OTP region:"`
	local otp_code=`echo $otp_dmesg | cut -d':' -f3 | tr -d ' '`
	local otp_region=""
	...
	*) otp_region="??";;
	...
	LogWrite "INFO|WiFi|Firmware OTP region code:${otp_code} ( ${otp_region} )"
...
function LogWrite_WiFiParameters() {
	...
	if [ -d "/sys/module/lrdmwl/parameters" ]; then 
		...
		LogWrite "INFO|WiFi|SISO_mode=${siso_mode}, ant_gain_adjust=${ant_gain_adjust}"
		return 1
	fi;
...
function LogWrite_WiFiChangeAP() {
    ...
    then
        ...
        if [ $prevmac != $mac ]
        then
            LogWrite "INFO|WiFi|disconnected"
            prevmac="none"
            return 0
        fi
    else
        ...
        if [ $prevmac != $mac ]
        then
            LogWrite "INFO|WiFi|connect to ${ssid} ${mac} ${dbm} dBm ${txbr}"
            prevmac=$mac
            return 1
        fi
    fi
...
function LogWrite_WiFiSignalPol() {
	local str=`wpa_cli -i wlp1s0 signal_pol`
...
function LogWrite_WiFiInfo() {
	...
	if [[ ${str} =~ "txpower" ]]; then
		if [[ ${str} =~ "channel" ]]; then
			...
			LogWrite "INFO|WiFi|${txpower} ${channel}"
		else
			str_tx=`echo "${str#*txpower }"`
			txpower=`echo $str_tx | awk '{print "txpower " $1, $2}'`
			LogWrite "INFO|WiFi|${txpower}"
		fi
...
sleep 0.5
LogWrite  "wifilog start!" 
iw event -t >> "${_root_path}/wificheck.log" &
