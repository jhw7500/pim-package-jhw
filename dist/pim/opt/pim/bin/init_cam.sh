...
no_wait=0
while [ $# -gt 0 ]; do
    case "$1" in -q|--quiet) shift;; --no-wait) no_wait=1; shift;; -h|--help) echo 'usage: init_cam.sh [-q] [--no-wait]'; exit 0;; *) echo "unknown option: $1" >&2; exit 64;; esac
done
echo 'DEPRECATED: init_cam.sh forwards one recovery request' >&2
if [ "$no_wait" -eq 1 ]; then
    exec "${PIM_CAMERA_RECOVERYCTL:-/opt/pim/bin/cam-recoveryctl}" request module_reload --source legacy-init-cam --reason legacy-wrapper
fi
